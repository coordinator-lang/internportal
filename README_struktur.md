...
│   ├── Städföretag_v6.html     ← REGISTER & ARBETSVERKTYG (stor ändring): +27 leverantörer (64 st), 72 kommuner i regiongrupper, leverantörskort med status/pris/tider/tillägg/rotation/tvätt/access/rapportering, uppdragshistorik med betyg, jämförelsevy, kommun- och företagssök. Ifyllda uppgifter sparas i Supabase (tabell stad_leverantorer) med localStorage som reserv
│   └── Städföretag_v6.1.html   ← inloggning krävs (portal-auth.js) (liten ändring)  ← AKTUELL
│
├── Nordicta_Protocol/
...
│
└── SOP/
    └── sop.html                         ← STABILT FILNAMN, DATALÅST (byggs från _src/SOP/sop.html). Sökbar SOP 2026, portal-stil, emoji-fri. v1.5 = 7 delar + FLÖDEN (DEL 6 = Facebook-sourcing; DEL 7 = Visningar kund/HV, inkl. "På plats"-rutin). Gamla sop_v1.x/sop-alt = kvarvarande arkiv, ej länkade.
```
